...
namespace latinime {
    ...
    public:
        ProximityInfo(JNIEnv *env, const int keyboardWidth, const int keyboardHeight,
                      ...
                      const jintArray proximityChars, const int keyCount,
                      const jintArray keyXCoordinates,
                      const jintArray keyYCoordinates, const jintArray keyWidths,
                      const jintArray keyHeights,
        ...
        AK_FORCE_INLINE void initializeProximities(const int *const inputCodes,
                                                   ...
                                                   const int inputSize, int *allInputCodes,
                                                   const std::vector<int> *locale) const {
            ProximityInfoUtils::initializeProximities(inputCodes, inputXCoordinates,
                                                      inputYCoordinates,
                                                      inputSize, mKeyXCoordinates, mKeyYCoordinates,
                                                      mKeyWidths, mKeyHeights,
                                                      mProximityCharsArray, CELL_HEIGHT, CELL_WIDTH,
                                                      GRID_WIDTH, MOST_COMMON_KEY_WIDTH,
                                                      KEY_COUNT, locale, &mLowerCodePointToKeyMap,
                                                      allInputCodes);
        }
