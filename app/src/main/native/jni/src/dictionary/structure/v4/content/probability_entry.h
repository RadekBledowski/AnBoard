...
namespace latinime {
    ...
    public:
        ...
        // Dummy entry
        ProbabilityEntry()
                : mFlags(Ver4DictConstants::FLAG_NOT_A_VALID_ENTRY),
        ...
        ProbabilityEntry(const int flags, const HistoricalInfo *const historicalInfo)
                : mFlags(flags), mProbability(NOT_A_PROBABILITY),
        ...
        uint64_t encode(const bool hasHistoricalInfo) const {
            ...
            if (hasHistoricalInfo) {
                ...
                encodedEntry =
                        (encodedEntry << (Ver4DictConstants::WORD_LEVEL_FIELD_SIZE * CHAR_BIT))
                        | static_cast<uint8_t>(mHistoricalInfo.getLevel());
                encodedEntry =
                        (encodedEntry << (Ver4DictConstants::WORD_COUNT_FIELD_SIZE * CHAR_BIT))
    ...
    private:
        ...
        static int
        readFromEncodedEntry(const uint64_t encodedEntry, const int size, const int pos) {
            return static_cast<int>(
                    (encodedEntry >> (pos * CHAR_BIT)) & ((1ull << (size * CHAR_BIT)) - 1));
        }

        static uint8_t createFlags(const bool representsBeginningOfSentence,
                                   const bool isNotAWord, const bool isBlacklisted,
