...
namespace latinime {
    ...
    float
    getEditDistance(const std::vector<int> &codePoints0, const std::vector<int> &codePoints1) {
        DamerauLevenshteinEditDistancePolicy policy(codePoints0.data(), codePoints0.size(),
                                                    codePoints1.data(), codePoints1.size());
...
EXPECT_FLOAT_EQ(1.0f, getEditDistance({
1 }, {
}));
EXPECT_FLOAT_EQ(1.0f, getEditDistance({
}, {
100 }));
EXPECT_FLOAT_EQ(5.0f, getEditDistance({
}, {
1, 2, 3, 4, 5 }));
...
1 }, {
1, 2 }));
EXPECT_FLOAT_EQ(2.0f, getEditDistance({
1, 2 }, {
0, 1, 2, 3 }));
EXPECT_FLOAT_EQ(2.0f, getEditDistance({
0, 1, 2, 3 }, {
