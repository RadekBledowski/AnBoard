...
namespace latinime {
    ...
    bool ShortcutDictContent::runGC(
        ...
        for (TerminalPositionLookupTable::TerminalIdMap::const_iterator it = terminalIdMap->begin();
            ...
            if (!copyShortcutListFromDictContent(originalShortcutListPos,
                                                 originalShortcutDictContent,
                                                 shortcutListPos)) {
    ...
    bool ShortcutDictContent::copyShortcutListFromDictContent(const int shortcutListPos,
                                                              const ShortcutDictContent *const sourceShortcutDictContent,
                                                              const int toPos) {
        bool hasNext = true;
    ...
    bool ShortcutDictContent::writeShortcutEntryAndAdvancePosition(const int *const codePoint,
        ...
        if (!shortcutListBuffer->writeUintAndAdvancePosition(shortcutFlags,
                                                             Ver4DictConstants::SHORTCUT_FLAGS_FIELD_SIZE,
                                                             shortcutEntryPos)) {
            AKLOGE("Cannot write shortcut flags. flags; %x, pos: %d", shortcutFlags,
                   *shortcutEntryPos);
            return false;
        }
        if (!shortcutListBuffer->writeCodePointsAndAdvancePosition(codePoint, codePointCount,
                                                                   true /* writesTerminator */,
                                                                   shortcutEntryPos)) {
...
// Find a shortcut entry that has specified target and return its position.
    int ShortcutDictContent::findShortcutEntryAndGetPos(const int shortcutListPos,
                                                        const int *const targetCodePointsToFind,
        ...
        while (hasNext) {
            ...
            getShortcutEntryAndAdvancePosition(MAX_WORD_LENGTH, targetCodePoints,
