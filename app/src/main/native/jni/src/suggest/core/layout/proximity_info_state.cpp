...
// TODO: Remove the dependency of "isGeometric"
    void ProximityInfoState::initInputParams(const int pointerId, const float maxPointToKeyLength,
                                             ...
                                             const int *const inputCodes, const int inputSize,
        ...
        if (xCoordinates && yCoordinates) {
            mSampledInputSize = ProximityInfoStateUtils::updateTouchPoints(mProximityInfo,
                                                                           ...
                                                                           xCoordinates,
                                                                           yCoordinates, times,
                                                                           pointerIds, inputSize,
                                                                           isGeometric, pointerId,
        ...
        if (mSampledInputSize > 0) {
            ProximityInfoStateUtils::initGeometricDistanceInfos(mProximityInfo, mSampledInputSize,
                                                                lastSavedInputSize, isGeometric,
            ...
            if (isGeometric) {
                ...
                mMostProbableStringProbability = ProximityInfoStateUtils::getMostProbableString(
                        mProximityInfo, mSampledInputSize, &mCharProbabilities,
                        mMostProbableString);
        ...
        if (DEBUG_GEO_FULL) {
            AKLOGI("ProximityState init finished: %d points out of %d", mSampledInputSize,
                   inputSize);
        }
...
// to the non-accented version.
    ...
    ProximityType
    ProximityInfoState::getProximityTypeG(const int index, const int codePoint) const {
        if (!isUsed()) {
...
// Returns a probability of mapping index to keyIndex.
    float ProximityInfoState::getProbability(const int index, const int keyIndex) const {
        ASSERT(0 <= index && index < mSampledInputSize);
        std::unordered_map<int, float>::const_iterator it = mCharProbabilities[index].find(
                keyIndex);
