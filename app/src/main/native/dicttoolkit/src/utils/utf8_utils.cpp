...
/* static */ std::string Utf8Utils::getUtf8String(const CodePointArrayView codePoints) {
            ...
            for (const int codePoint : codePoints) {
                ...
                // Output first byte.
                const int value =
                        codePoint >> (trailingByteCount * CODE_POINT_BIT_COUNT_IN_TRAILING_BYTE);
                ...
                for (int i = 1; i < sequenceSize; ++i) {
                    const int shiftAmount =
                            (trailingByteCount - i) * CODE_POINT_BIT_COUNT_IN_TRAILING_BYTE;
                    const int value =
                            (codePoint >> shiftAmount) & TRAILING_BYTE_CODE_POINT_BITS_MASK;
