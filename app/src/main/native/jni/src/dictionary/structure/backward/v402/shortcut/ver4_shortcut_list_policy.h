    ...
    namespace backward {
        namespace v402 {
            ...
            public:
                ...
                bool addNewShortcut(const int terminalId, const int *const codePoints,
                                    const int codePointCount,
                                    const int probability) {
                    const int shortcutListPos = mShortcutDictContent->getShortcutListHeadPos(
                            terminalId);
                    if (shortcutListPos == NOT_A_DICT_POS) {
                        ...
                        const int writingPos = mShortcutDictContent->getShortcutListHeadPos(
                                terminalId);
                    ...
                    const int entryPos = mShortcutDictContent->findShortcutEntryAndGetPos(
                            shortcutListPos,
                    ...
                    if (entryPos == NOT_A_DICT_POS) {
                        ...
                        if (!mShortcutDictContent->writeShortcutEntryAndAdvancePosition(codePoints,
                                                                                        ...
                                                                                        true /* hasNext */,
                                                                                        &writingPos)) {
                            AKLOGE("Cannot write shortcut entry. terminal id: %d, pos: %d",
                                   terminalId,
                                   writingPos);
                            return false;
                        }
                    ...
                    mShortcutDictContent->getShortcutEntry(MAX_WORD_LENGTH, 0 /* outCodePoint */,
                                                           0 /* outCodePointCount */ ,
                                                           0 /* probability */, &hasNext, entryPos);
                    if (!mShortcutDictContent->writeShortcutEntry(codePoints,
                                                                  codePointCount, probability,
                                                                  hasNext, entryPos)) {
                        AKLOGE("Cannot overwrite shortcut entry. terminal id: %d, pos: %d",
                               terminalId,
                               entryPos);
                        return false;
                    }
