...
namespace latinime {
    namespace {
        ...
        EXPECT_FALSE(trieMap
        .get(11, next).mIsValid);
...
EXPECT_FALSE(result
.mIsValid);
EXPECT_EQ(TrieMap::INVALID_INDEX, result
...
std::map<int, std::map<int, std::map < int, uint64_t>>>
threeLevelMap;
...
firstLevelKeys.
push_back(key);
firstLevelEntries[key] =
value;
}

for (
int i = 0;
i<SECOND_LEVEL_ENTRY_COUNT;
++i) {
const int key = keyRandomNumberGenerator();
const uint64_t value = valueRandomNumberGenerator();
...
EXPECT_TRUE(trieMap
.
put(key, value, nextLevelBitmapEntryIndex
));
secondLevelKeys.
push_back(std::make_pair(firstLevelKey, key)
);
twoLevelMap[firstLevelKey][key] =
value;
}

for (
int i = 0;
i<THIRD_LEVEL_ENTRY_COUNT;
++i) {
const int key = keyRandomNumberGenerator();
const uint64_t value = valueRandomNumberGenerator();
...
EXPECT_TRUE(trieMap
.
put(key, value, thirdLevel
));
threeLevelMap[secondLevelKey.first][secondLevelKey.second][key] =
value;
}

for (
const auto &firstLevelEntry
: firstLevelEntries) {
EXPECT_EQ(firstLevelEntry
.second, trieMap.
getRoot(firstLevelEntry
.first).mValue);
}
...
for (
const auto &secondLevelEntry
: firstLevelEntry.second) {
EXPECT_EQ(secondLevelEntry
.second,
trieMap.
get(secondLevelEntry
.first, secondLevel).mValue);
}
}
...
for (
const auto &thirdLevelEntry
: secondLevelEntry.second) {
EXPECT_EQ(thirdLevelEntry
.second,
trieMap.
get(thirdLevelEntry
.first, thirdLevel).mValue);
}
}
}
...
).mValue, firstLevelEntry.

value()

);
EXPECT_EQ(firstLevelEntries[firstLevelEntry.key()], firstLevelEntry
.
...
twoLevelMap[firstLevelEntry.

key()

].
erase(secondLevelEntry
.
...
[thirdLevelEntry.key()], thirdLevelEntry
.

value()

);
threeLevelMap[firstLevelEntry.
...
EXPECT_EQ(testKeyValuePairs[entry.key()], entry
.
