        ...
        namespace v402 {
            ...
            bool Ver4PatriciaTrieNodeWriter::markPtNodeAsMoved(
                ...
                if (toBeUpdatedPtNodeParams->hasChildren()) {
                    ...
                    mReadingHelper.initWithPtNodeArrayPos(
            ...
            bool Ver4PatriciaTrieNodeWriter::markPtNodeAsWillBecomeNonTerminal(
                ...
                // Update flags.
                int writingPos = toBeUpdatedPtNodeParams->getHeadPos();
                return DynamicPtWritingUtils::writeFlagsAndAdvancePosition(mTrieBuffer,
                                                                           updatedFlags,
                                                                           &writingPos);
            }
            ...
            bool Ver4PatriciaTrieNodeWriter::updatePtNodeUnigramProperty(
                ...
                const ProbabilityEntry originalProbabilityEntry =
                        mBuffers->getProbabilityDictContent()->getProbabilityEntry(
                                toBeUpdatedPtNodeParams->getTerminalId());
                const ProbabilityEntry probabilityEntry = createUpdatedEntryFrom(
            ...
            bool Ver4PatriciaTrieNodeWriter::updatePtNodeProbabilityAndGetNeedsToKeepPtNodeAfterGC(
                ...
                if (originalProbabilityEntry.hasHistoricalInfo()) {
                    ...
                    const ProbabilityEntry probabilityEntry =
                            originalProbabilityEntry.createEntryWithUpdatedHistoricalInfo(
                                    &historicalInfo);
            ...
            bool Ver4PatriciaTrieNodeWriter::updateTerminalId(
                    const PtNodeParams *const toBeUpdatedPtNodeParams,
                    const int newTerminalId) {
                return mTrieBuffer->writeUint(newTerminalId,
            ...
            bool Ver4PatriciaTrieNodeWriter::writePtNodeAndAdvancePosition(
                ...
                return writePtNodeAndGetTerminalIdAndAdvancePosition(ptNodeParams,
                                                                     0 /* outTerminalId */,
                                                                     ptNodeWritingPos);
            }
            ...
            bool Ver4PatriciaTrieNodeWriter::addNgramEntry(const WordIdArrayView prevWordIds,
                                                           const int wordId,
                                                           const NgramProperty *const ngramProperty,
                                                           bool *const outAddedNewEntry) {
                if (!mBigramPolicy->addNewEntry(prevWordIds[0], wordId, ngramProperty,
                                                outAddedNewEntry)) {
                ...
                if (!sourcePtNodeParams.hasBigrams()) {
                    ...
                    return updatePtNodeFlags(sourcePtNodeParams.getHeadPos(),
                                             sourcePtNodeParams.isPossiblyOffensive(),
                                             sourcePtNodeParams.isNotAWord(),
                                             sourcePtNodeParams.isTerminal(),
                                             sourcePtNodeParams.hasShortcutTargets(),
                                             true /* hasBigrams */,
                                             sourcePtNodeParams.getCodePointCount() >
                                             1 /* hasMultipleChars */);
                }
                return true;
            }
            ...
            bool Ver4PatriciaTrieNodeWriter::updateAllPositionFields(
                ...
                if (childrenPos != NOT_A_DICT_POS) {
                    PtNodeWriter::PtNodeArrayPositionRelocationMap::const_iterator it =
                            dictPositionRelocationMap->mPtNodeArrayPositionRelocationMap.find(
                                    childrenPos);
            ...
            bool
            Ver4PatriciaTrieNodeWriter::addShortcutTarget(const PtNodeParams *const ptNodeParams,
                ...
                if (!mShortcutPolicy->addNewShortcut(ptNodeParams->getTerminalId(),
                                                     ...
                                                     shortcutProbability)) {
                    AKLOGE("Cannot add new shortcut entry. terminalId: %d",
                ...
                if (!ptNodeParams->hasShortcutTargets()) {
                    ...
                    return updatePtNodeFlags(ptNodeParams->getHeadPos(),
                                             ...
                                             ptNodeParams->isTerminal(),
                                             true /* hasShortcutTargets */,
            ...
            bool Ver4PatriciaTrieNodeWriter::updatePtNodeHasBigramsAndShortcutTargetsFlags(
                ...
                const bool hasShortcutTargets =
                        mBuffers->getShortcutDictContent()->getShortcutListHeadPos(
                                ptNodeParams->getTerminalId()) != NOT_A_DICT_POS;
                return updatePtNodeFlags(ptNodeParams->getHeadPos(),
                                         ptNodeParams->isPossiblyOffensive(),
                                         ptNodeParams->isNotAWord(), ptNodeParams->isTerminal(),
                                         hasShortcutTargets,
                                         hasBigrams, ptNodeParams->getCodePointCount() >
                                                     1 /* hasMultipleChars */);
            }
            ...
            bool Ver4PatriciaTrieNodeWriter::writePtNodeAndGetTerminalIdAndAdvancePosition(
                ...
                return updatePtNodeFlags(nodePos, ptNodeParams->isPossiblyOffensive(),
                                         ...
                                         ptNodeParams->hasShortcutTargets(),
                                         ptNodeParams->hasBigrams(),
                                         ptNodeParams->getCodePointCount() >
                                         1 /* hasMultipleChars */);
            }
            ...
            bool Ver4PatriciaTrieNodeWriter::suppressUnigramEntry(
                    const PtNodeParams *const ptNodeParams) {
                if (!mHeaderPolicy->hasHistoricalInfoOfWords()) {
                    // Require historical info to suppress unigram entry.
                    return false;
                }
                const HistoricalInfo suppressedHistorycalInfo(0 /* timestamp */, 0 /* level */,
                                                              0 /* count */);
                const ProbabilityEntry probabilityEntryToWrite =
                        ProbabilityEntry().createEntryWithUpdatedHistoricalInfo(
                                &suppressedHistorycalInfo);
                return mBuffers->getMutableProbabilityDictContent()->setProbabilityEntry(
