...
// TODO: Needs to allocate larger memory corresponding to the current vector size.
    ...
    uint32_t BufferWithExtendableBuffer::readUint(const int size, const int pos) const {
        ...
        return ByteArrayUtils::readUint(getBuffer(readingPosIsInAdditionalBuffer), size,
                                        posInBuffer);
    }
    ...
    bool
    BufferWithExtendableBuffer::writeUintAndAdvancePosition(const uint32_t data, const int size,
                                                            int *const pos) {
        if (!(size >= 1 && size <= 4)) {
