...
namespace latinime {
    ...
    public:
        static void
        jintarrayToVector(JNIEnv *env, jintArray array, std::vector<int> *const outVector) {
            if (!array) {
                outVector->clear();
                return;
            }
        ...
        static DictionaryHeaderStructurePolicy::AttributeMap constructAttributeMap(JNIEnv *env,
            ...
            for (int i = 0; i < keyCount; i++) {
                ...
                env->GetStringUTFRegion(valueString, 0, env->GetStringLength(valueString),
                                        valueChars);
        ...
        static void
        outputCodePoints(JNIEnv *env, jintArray intArrayToOutputCodePoints, const int start,
        ...
        static NgramContext constructNgramContext(JNIEnv *env, jobjectArray prevWordCodePointArrays,
            ...
            for (size_t i = 0; i < prevWordCount; ++i) {
                ...
                jintArray prevWord = (jintArray) env->GetObjectArrayElement(prevWordCodePointArrays,
                                                                            i);
                if (!prevWord) {
                    continue;
                }
