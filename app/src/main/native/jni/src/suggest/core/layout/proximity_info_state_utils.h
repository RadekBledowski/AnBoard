...
namespace latinime {
    ...
    public:
        ...
        static int updateTouchPoints(const ProximityInfo *const proximityInfo,
                                     ...
                                     const int inputSize,
                                     const bool isGeometric, const int pointerId,
                                     const int pushTouchPointStartIndex,
        ...
        static const int *
        getProximityCodePointsAt(const int *const inputProximities, const int index);
        ...
        static void refreshBeelineSpeedRates(const int mostCommonKeyWidth, const float averageSpeed,
                                             const int inputSize, const int *const xCoordinates,
        ...
        static float getDirection(const std::vector<int> *const sampledInputXs,
                                  const std::vector<int> *const sampledInputYs, const int index0,
                                  const int index1);

        static void updateAlignPointProbabilities(const float maxPointToKeyLength,
                                                  const int mostCommonKeyWidth, const int keyCount,
                                                  const int start,
                                                  const int sampledInputSize,
                                                  const std::vector<int> *const sampledInputXs,
                                                  const std::vector<int> *const sampledInputYs,
    ...
    private:
        ...
        static float updateNearKeysDistances(const ProximityInfo *const proximityInfo,
                                             const float maxPointToKeyLength, const int x,
                                             const int y,
        ...
        static float getPointScore(const int mostCommonKeyWidth, const int x, const int y,
                                   const int time, const bool lastPoint, const float nearest,
        ...
        static bool pushTouchPoint(const ProximityInfo *const proximityInfo,
                                   const int maxPointToKeyLength, const int inputIndex,
                                   const int nodeCodePoint, int x,
        ...
        static float
        calculateBeelineSpeedRate(const int mostCommonKeyWidth, const float averageSpeed,
                                  const int id, const int inputSize, const int *const xCoordinates,
                                  const int *const yCoordinates, const int *times,
                                  const int sampledInputSize,
                                  const std::vector<int> *const sampledInputXs,
                                  const std::vector<int> *const sampledInputYs,
                                  const std::vector<int> *const inputIndice);
