...
namespace latinime {
    namespace {
        ...
        EXPECT_EQ(0x6f,
        ByteArrayUtils::readCodePointAndAdvancePosition(buffer, codePointTable, &pos
        ));
    ...
    EXPECT_EQ(0x01u,
    ByteArrayUtils::readUint8(buffer,
    0));
    EXPECT_EQ(0x8Au,
    ByteArrayUtils::readUint8(buffer,
    1));
    ...
    EXPECT_EQ(0x018Au,
    ByteArrayUtils::readUint16(buffer,
    0));
    EXPECT_EQ(0x8A00u,
    ByteArrayUtils::readUint16(buffer,
    1));
    ...
    EXPECT_EQ(0x18A00AAu,
    ByteArrayUtils::readUint32(buffer,
    0));

    int pos = 0;
    EXPECT_EQ(0x18A00,
    ByteArrayUtils::readSint24AndAdvancePosition(buffer, &pos
    ));
    pos = 1;
    EXPECT_EQ(-0xA00AA,
    ByteArrayUtils::readSint24AndAdvancePosition(buffer, &pos
...
ByteArrayUtils::writeUintAndAdvancePosition(buffer, data_1B,
1, &pos);
...
ByteArrayUtils::writeUintAndAdvancePosition(buffer, data_4B,
4, &pos);
...
EXPECT_EQ(0x10FF00,
ByteArrayUtils::readCodePoint(buffer,
0));
EXPECT_EQ(0x20,
ByteArrayUtils::readCodePoint(buffer,
3));
...
EXPECT_EQ(3,
ByteArrayUtils::readStringAndAdvancePosition(buffer, MAX_WORD_LENGTH,
nullptr,
codePointArray, &pos));
...
EXPECT_EQ(0x60,
ByteArrayUtils::readCodePoint(buffer, pos
));
}
...
int pos = 0;
ByteArrayUtils::writeCodePointsAndAdvancePosition(buffer, codePointArray,
3,
