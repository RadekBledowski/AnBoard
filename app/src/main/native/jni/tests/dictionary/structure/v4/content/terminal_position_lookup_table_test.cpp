...
namespace latinime {
    namespace {
        ...
        EXPECT_EQ(NOT_A_DICT_POS, lookupTable
        .getTerminalPtNodePosition(0));
        EXPECT_EQ(NOT_A_DICT_POS, lookupTable
        .getTerminalPtNodePosition(-1));
        EXPECT_EQ(NOT_A_DICT_POS, lookupTable
        .
        getTerminalPtNodePosition(
    ...
    .setTerminalPtNodePosition(10, 100));
    EXPECT_EQ(100, lookupTable.getTerminalPtNodePosition(10));
    EXPECT_EQ(NOT_A_DICT_POS, lookupTable
    .getTerminalPtNodePosition(9));
    EXPECT_TRUE(lookupTable
    .setTerminalPtNodePosition(9, 200));
    EXPECT_EQ(200, lookupTable.getTerminalPtNodePosition(9));
    EXPECT_TRUE(lookupTable
    .setTerminalPtNodePosition(10, 300));
    EXPECT_EQ(300, lookupTable.getTerminalPtNodePosition(10));
    EXPECT_FALSE(lookupTable
    .setTerminalPtNodePosition(-1, 400));
    EXPECT_EQ(NOT_A_DICT_POS, lookupTable
    .getTerminalPtNodePosition(-1));
    EXPECT_FALSE(lookupTable
    .
    setTerminalPtNodePosition(Ver4DictConstants::NOT_A_TERMINAL_ID,
    500));
    EXPECT_EQ(NOT_A_DICT_POS, lookupTable
    .
    getTerminalPtNodePosition(
...
EXPECT_TRUE(lookupTable
.
setTerminalPtNodePosition(terminalIds[i], terminalPositions[i]
));
}
...
EXPECT_TRUE(lookupTable
.
runGCTerminalIds(&terminalIdMap)
);
...
<int>(i), terminalIdMap[terminalIds[i]]
)
<< "Terminal id (" << terminalIds[i] << ") should be changed to " <<
i;
EXPECT_EQ(terminalPositions[i], lookupTable
.
getTerminalPtNodePosition(i)
);
}
}
