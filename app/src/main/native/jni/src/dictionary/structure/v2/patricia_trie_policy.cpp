    ...
    int PatriciaTriePolicy::getCodePointsAndProbabilityAndReturnCodePointCount(
        ...
        for (int loopCount = maxCodePointCount; loopCount > 0; --loopCount) {
            ...
            for (int ptNodeCount = PatriciaTrieReadingUtils::getPtNodeArraySizeAndAdvancePosition(
                ...
                if (!isValidPos(pos)) {
                    AKLOGE("PtNode position is invalid. pos: %d, dict size: %zd", pos,
                           mBuffer.size());
                    mIsCorrupted = true;
                    ASSERT(false);
                    return 0;
                    ...
                    if (0 != lastCandidatePtNodePos) {
                        ...
                        if (PatriciaTrieReadingUtils::hasBigrams(flags)) {
                            if (!mBigramListPolicy.skipAllBigrams(&pos)) {
                                AKLOGE("Cannot skip bigrams. BufSize: %zd, pos: %d.",
                                       mBuffer.size(),
                                       pos);
                                mIsCorrupted = true;
                                ASSERT(false);
...
// dictionary. If no match is found, it returns NOT_A_WORD_ID.
    ...
    PatriciaTriePolicy::getShortcutIterator(const int wordId) const {
    ...
    int PatriciaTriePolicy::createAndGetLeavingChildNode(const DicNode *const dicNode,
        ...
        PatriciaTrieReadingUtils::readPtNodeInfo(mBuffer.data(), ptNodePos, &mShortcutListPolicy,
                                                 &mBigramListPolicy, codePointTable, &flags,
                                                 &mergedNodeCodePointCount,
                                                 mergedNodeCodePoints, &probability, &childrenPos,
                                                 &shortcutPos, &bigramPos,
                                                 &siblingPos);
        ...
        if (CharUtils::isInUnicodeSpace(mergedNodeCodePoints[0])) {
            const int wordId = PatriciaTrieReadingUtils::isTerminal(flags) ? ptNodePos
                                                                           : NOT_A_WORD_ID;
            childDicNodes->pushLeavingChild(dicNode, childrenPos, wordId,
                                            CodePointArrayView(mergedNodeCodePoints,
                                                               mergedNodeCodePointCount));
        }
        return siblingPos;
    }
    ...
    const WordProperty PatriciaTriePolicy::getWordProperty(
        ...
        while (bigramsIt.hasNext()) {
            ...
            if (bigramsIt.getBigramPos() != NOT_A_DICT_POS) {
                ...
                const int probability = getProbability(word1Probability,
                                                       bigramsIt.getProbability());
                ngrams.emplace_back(
        ...
        const UnigramProperty unigramProperty(ptNodeParams.representsBeginningOfSentence(),
                                              ...
                                              ptNodeParams.isPossiblyOffensive(),
                                              ptNodeParams.getProbability(), HistoricalInfo(),
                                              std::move(shortcuts));
