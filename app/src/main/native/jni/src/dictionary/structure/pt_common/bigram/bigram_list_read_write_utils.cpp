...
/* static */ bool BigramListReadWriteUtils::getBigramEntryPropertiesAndAdvancePosition(
        ...
        const int targetPos = getBigramAddressAndAdvancePosition(buffer, bigramFlags,
                                                                 bigramEntryPos);
...
/* static */ bool BigramListReadWriteUtils::skipExistingBigrams(const ReadOnlyByteArrayView buffer,
        ...
        BigramFlags flags;
        do {
            if (!getBigramEntryPropertiesAndAdvancePosition(buffer, &flags,
                                                            0 /* outTargetPtNodePos */,
                                                            bigramListPos)) {
                return false;
            }
        } while (hasNext(flags));
