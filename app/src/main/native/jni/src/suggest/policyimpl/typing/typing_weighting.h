...
namespace latinime {
    ...
    protected:
        ...
        float
        getOmissionCost(const DicNode *const parentDicNode, const DicNode *const dicNode) const {
        ...
        float getInsertionCost(const DicTraverseSession *const traverseSession,
            ...
            const int prevCodePoint = traverseSession->getProximityInfoState(
                    0)->getPrimaryCodePointAt(
                    insertedPointIndex);
