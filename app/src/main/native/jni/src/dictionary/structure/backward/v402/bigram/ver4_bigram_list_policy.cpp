...
namespace latinime {
    namespace backward {
        namespace v402 {
            ...
            void
            Ver4BigramListPolicy::getNextBigram(int *const outBigramPos, int *const outProbability,
            ...
            bool
            Ver4BigramListPolicy::addNewEntry(const int terminalId, const int newTargetTerminalId,
                ...
                if (tailEntryPos != NOT_A_DICT_POS || entryPosToUpdate == NOT_A_DICT_POS) {
                    ...
                    if (tailEntryPos == NOT_A_DICT_POS) {
                        ...
                        if (!mBigramDictContent->copyBigramList(bigramListPos, destPos,
                                                                &tailEntryPos)) {
                            return false;
                        }
                    }
                ...
                const BigramEntry originalBigramEntry = mBigramDictContent->getBigramEntry(
                        entryPosToUpdate);
            ...
            bool
            Ver4BigramListPolicy::removeEntry(const int terminalId, const int targetTerminalId) {
                const int bigramListPos = mBigramDictContent->getBigramListHeadPos(terminalId);
                if (bigramListPos == NOT_A_DICT_POS) {
                ...
                const BigramEntry bigramEntry = mBigramDictContent->getBigramEntry(
                        entryPosToUpdate);
            ...
            bool Ver4BigramListPolicy::updateAllBigramEntriesAndDeleteUselessEntries(
                ...
                while (hasNext) {
                    ...
                    if (targetPtNodePos == NOT_A_DICT_POS) {
                        ...
                        if (ForgettingCurveUtils::needsToKeep(&historicalInfo, mHeaderPolicy)) {
                            ...
                            if (!mBigramDictContent->writeBigramEntry(&updatedBigramEntry,
                                                                      entryPos)) {
                                return false;
                            }
                            *outBigramCount += 1;
                ...
                if (mHeaderPolicy->hasHistoricalInfoOfWords()) {
                    ...
                    const HistoricalInfo updatedHistoricalInfo =
                            ForgettingCurveUtils::createUpdatedHistoricalInfo(
                                    originalBigramEntry->getHistoricalInfo(),
                                    ngramProperty->getProbability(),
                                    &historicalInfoForUpdate, mHeaderPolicy);
                    return originalBigramEntry->updateHistoricalInfoAndGetEntry(
                            &updatedHistoricalInfo);
                } else {
                    return originalBigramEntry->updateProbabilityAndGetEntry(
                            ngramProperty->getProbability());
                }
            }

            bool
            Ver4BigramListPolicy::updateHasNextFlag(const bool hasNext, const int bigramEntryPos) {
                const BigramEntry bigramEntry = mBigramDictContent->getBigramEntry(bigramEntryPos);
                const BigramEntry updatedBigramEntry = bigramEntry.updateHasNextAndGetEntry(
                        hasNext);
