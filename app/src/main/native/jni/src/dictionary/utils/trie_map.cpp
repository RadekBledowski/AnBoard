 ...
 */
    const TrieMap::Result
    TrieMap::iterateNext(std::vector<TableIterationState> *const iterationState,
        ...
        while (!iterationState->empty()) {
            ...
            if (state.mTableSize <= state.mCurrentIndex) {
                ...
                if (entry.isBitmapEntry()) {
                    // Move to child.
                    iterationState->emplace_back(popCount(entry.getBitmap()),
                                                 entry.getTableIndex());
 ...
 */
    ...
    int TrieMap::getTerminalEntryIndex(const uint32_t key, const uint32_t hashedKey,
        ...
        const int entryIndex =
                bitmapEntry.getTableIndex() + popCount(bitmapEntry.getBitmap(), label);
        const Entry entry = readEntry(entryIndex);
        if (entry.isBitmapEntry()) {
 ...
 */
    bool TrieMap::putInternal(const uint32_t key, const uint64_t value, const uint32_t hashedKey,
                              const int bitmapEntryIndex, const Entry &bitmapEntry,
                              const int level) {
        const int label = getLabel(hashedKey, level);
        ...
        if (!exists(bitmap, label)) {
            ...
            return addNewEntryByExpandingTable(key, value, mapIndex, bitmap, bitmapEntryIndex,
                                               label);
        }
 ...
 */
    bool TrieMap::addNewEntryByResolvingConflict(const uint32_t key, const uint64_t value,
        ...
        if (conflictedKeyNextLabel == nextLabel) {
            ...
            return putInternal(key, value, hashedKey, conflictedEntryIndex, newBitmapEntry,
                               level + 1);
        }
 ...
 */
    bool TrieMap::addNewEntryByExpandingTable(const uint32_t key, const uint64_t value,
                                              const int tableIndex, const uint32_t bitmap,
                                              const int bitmapEntryIndex, const int label) {
        // Current map doesn't contain the label.
        ...
        for (int i = 0; i < entryCount; ++i) {
            if (!copyEntry(tableIndex + i,
                           newTableIndex + i + (i >= newEntryIndexInTable ? 1 : 0))) {
                return false;
            }
        }
