...
namespace latinime {
    namespace {
        ...
        setProbabilityEntry(wordId, &probabilityEntry
        );
        const ProbabilityEntry entry =
                languageModelDictContent.getProbabilityEntry(wordId);
        EXPECT_EQ(flag, entry
        .

        getFlags()

        );
        EXPECT_EQ(probability, entry
        .
        ...
        // Remove
        EXPECT_TRUE(languageModelDictContent
        .
        removeProbabilityEntry(wordId)
        );
        EXPECT_FALSE(languageModelDictContent
        .
        getProbabilityEntry(wordId)
        .
    ...
    EXPECT_EQ(timestamp, entry
    .getHistoricalInfo()->getTimestamp());
    EXPECT_EQ(count, entry
    .getHistoricalInfo()->getCount());
...
: wordIds) {
languageModelDictContent.
setProbabilityEntry(wordId, &originalEntry
);
}
...
getProbabilityEntries (WordIdArrayView())

) {
EXPECT_EQ(originalEntry
.
...
languageModelDictContent.
setProbabilityEntry(prevWordIds[0], &probabilityEntry
);
languageModelDictContent.
setNgramProbabilityEntry(prevWordIds
.limit(1), wordId,
&bigramProbabilityEntry);
EXPECT_EQ(bigramProbability, languageModelDictContent
.
getWordAttributes(prevWordIds, wordId,
false /* mustMatchAllPrevWords */, nullptr /* headerPolicy */).
...
EXPECT_EQ(trigramProbability, languageModelDictContent
.
getWordAttributes(prevWordIds, wordId,
false /* mustMatchAllPrevWords */, nullptr /* headerPolicy */).
